 ...
 * 无人机摄影测量作业：空间点求相机参数
 * 求解方程：x = K[R|T]X
 * 其中 x(图像点), K(内参矩阵), X(空间点) 已知
 * 求解 R(旋转矩阵) 和 T(平移向量)
 *
 * 算法步骤：
 * 1. 使用DLT方法求解线性方程，得到投影矩阵M = K[R|T]
 * 2. 通过矩阵分解从M中提取R和T
 */
...
// SVD分解（使用Jacobi方法求解对称矩阵的特征值和特征向量）
class SVD
{
public:
	Matrix U, S, V;
...
int main()
{
	...
	Matrix R_true(3, 3, {cos(angle), 0, sin(angle), 0, 1, 0, -sin(angle), 0, cos(angle)});
